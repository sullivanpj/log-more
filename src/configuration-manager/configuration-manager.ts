...
import { Configuration, CustomEventTypeDetails } from "src/types/configuration";
import { nanoid } from "nanoid";
import { DefaultConfiguration } from "src/constants/default-configuration";
import { Result } from "src/result";
import { DefaultEventTypeRegistry } from "src/constants/default-event-type-registry";
import { ResultValidationType } from "src/types/result";
import { writeError } from "src/lib/writeError";
import { IIndexable } from "src/types/utils";
...
export class ConfigurationManager {
    ...
    public static setConfig(params?: Partial<Configuration>): Result {
        try {
            ...
            config.eventTypeRegistry = Object.keys(
        ...
        try {
            ...
            if (customEventTypeDetails) {
                ...
                if (!customEventTypeDetails?.validationType) {
                    customEventTypeDetails.validationType =
                        defaultValidationType;
                }
                if (!customEventTypeDetails?.formatTemplate) {
                    customEventTypeDetails.formatTemplate =
                        defaultFormatTemplate;
                }
            ...
            writeError(
                `An error occurred retrieving event type registry: ${Result.error(
                    e
                )}`
            );
        }
