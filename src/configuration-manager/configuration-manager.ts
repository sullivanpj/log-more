import { Configuration, EventTypeDetails } from "../types/configuration";
...
import { writeError } from "../lib/writeError";
...
export class ConfigurationManager {
  ...
  public static setConfig(params?: Partial<Configuration>): Result {
    try {
      ...
      config.eventTypeRegistry = Object.keys(DefaultEventTypeRegistry).reduce(
        (ret: Record<string, EventTypeDetails>, key: string) => {
          if (
            !ret[key] ||
            !ret[key]?.formatTemplate ||
            !ret[key]?.writeLogEvent ||
            !ret[key]?.validationType
          ) {
            ret[key] = {
              formatTemplate: !!ret[key]?.formatTemplate
                ? ret[key]?.formatTemplate
                : config.defaultFormatTemplate,
              writeLogEvent: ret[key]?.writeLogEvent ?? config.writeLogEvent,
              validationType:
                ret[key]?.validationType ??
   ...
   * @returns The logging configuration object
   */
  public static getEventTypeRegistry(eventType: string): EventTypeDetails {
    const {
      eventTypeRegistry,
      writeLogEvent,
    ...
    try {
      const EventTypeDetails = eventTypeRegistry?.[eventType];
      if (EventTypeDetails) {
        if (!EventTypeDetails?.writeLogEvent) {
          EventTypeDetails.writeLogEvent = writeLogEvent;
        }
        if (!EventTypeDetails?.validationType) {
          EventTypeDetails.validationType = defaultValidationType;
        }
        if (!EventTypeDetails?.formatTemplate) {
          EventTypeDetails.formatTemplate = defaultFormatTemplate;
        }
